...
namespace GSWS.Assets.Tests.Database
{
    ...
    using GenericCollection = Collection<T>

    public class TestDatabase : GSWS.Assets.Database.Database
    {
        private Collection<Item> testCollection;
        private string testCollectionName = "testCollection";

        public override bool TryGetCollection<TColl>(string collectionName, out TColl collection)
        {
            if (collectionName == testCollectionName)
            {
                collection = (TColl)testCollection;
                return true;
            }
            return false;
        }
